...
pub enum ChannelMsg {
    ...
    /// Custom SSH channel request (Teleport reverse-tunnel dial / ping).
    CustomRequest {
        request: String,
        want_reply: bool,
        payload: Vec<u8>,
...
impl<S: From<(ChannelId, ChannelMsg)> + Send + Sync + 'static> ChannelWriteHalf<S> {
    ...
    pub async fn success(&self) -> Result<(), Error> {
        self.send_msg(ChannelMsg::Success).await
    }

    pub async fn failure(&self) -> Result<(), Error> {
        self.send_msg(ChannelMsg::Failure).await
    }

    /// Send a custom SSH channel request (`SSH_MSG_CHANNEL_REQUEST`).
    pub async fn request(
        ...
        self.send_msg(ChannelMsg::CustomRequest {
...
impl<S: From<(ChannelId, ChannelMsg)> + Send + Sync + 'static> Channel<S> {
    ...
    /// Send a custom SSH channel request.
    pub async fn request(
        &self,
        request: impl Into<String>,
        want_reply: bool,
        ...
        self.write_half.request(request, want_reply, payload).await
    }
    ...
    pub async fn success(&self) -> Result<(), Error> {
        self.write_half.success().await
    }

    pub async fn failure(&self) -> Result<(), Error> {
        self.write_half.failure().await
    }
