            ...
            Ok(pk_or_cert) => {
                ...
                let pubkey = match pk_or_cert {
                    ...
                    PublicKeyOrCertificate::CertificateRaw { .. } => {
                        warn!("raw certificates not accepted on russh server");
                        reject_auth_request(until, &mut self.write, auth_request).await?;
                        return Ok(());
                    }
                    ...
                    if is_valid {
                        ...
                        if SIGNATURE_BUFFER.with(|buf| {
                            ...
                            let auth = match pk_or_cert {
                                ...
                                PublicKeyOrCertificate::CertificateRaw { .. } => Auth::Reject {
            ...
            msg::CHANNEL_REQUEST => {
                ...
                match req_type.as_str() {
                    ...
                    "auth-agent-req@openssh.com" => {
                        ...
                        // This is a *channel* request. Global REQUEST_FAILURE
                        // desyncs Teleport/tsh and kills the web console PTY.
                        if response {
                    ...
                    x => {
                        // Teleport/OpenSSH send keepalive@openssh.com, eow@openssh.com,
                        // and x-teleport-* extras. CHANNEL_FAILURE aborts the PTY.
