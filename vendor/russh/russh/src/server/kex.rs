    ...
    pub async fn step<H: Handler + Send>(
        ...
        match self.state {
            ...
            ServerKexState::WaitingForDhInit { mut names, mut kex } => {
                ...
                let host_key_blob = if names.key.to_string().contains("-cert-") {
                    self.config
                        .host_cert
                        .clone()
                        .ok_or_else(|| Error::UnknownKey)?
                } else {
                    key.public_key().to_bytes().map_err(crate::Error::from)?
                };
                ...
                let hash = HASH_BUF.with(|buffer| {
                    ...
                    host_key_blob.encode(&mut pubkey_vec)?;
