...
pub(crate) fn is_key_compatible_with_algo(key: &PrivateKey, algo: &Algorithm) -> bool {
    match algo {
        ...
        a => {
            ...
            let s = a.to_string();
            if let Some(raw) = s.strip_suffix("-cert-v01@openssh.com") {
                return key.algorithm().to_string() == raw;
            }
            false
        }
    }
...
impl Preferred {
        ...
        self.key
            ...
            .filter(|n| {
                available_host_keys
                    ...
                    || {
                        // host certs are advertised via Config.host_cert_algo; included below
                        false
                    }
            })
            .cloned()
    ...
    pub(crate) fn possible_host_key_algos_for_server(
        &self,
        config: &Config,
    ) -> Vec<Algorithm> {
        let mut algos = self.possible_host_key_algos_for_keys(&config.keys);
        if let Some(algo) = &config.host_cert_algo {
            if !algos.contains(algo) {
                algos.insert(0, algo.clone());
            }
        }
        algos
    }
