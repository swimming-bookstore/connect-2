...
impl<H: Handler> Handle<H> {
    ...
    /// Perform OpenSSH certificate auth with a pre-encoded cert blob.
    /// Used for Teleport host certs whose timestamps `ssh-key` cannot parse.
    pub async fn authenticate_openssh_cert_raw<U: Into<String>>(
        &mut self,
        user: U,
        key: Arc<PrivateKey>,
        algo: String,
        ...
        self.sender
            .send(Msg::Authenticate {
                user,
                method: auth::Method::OpenSshCertificateRaw { key, algo, blob },
            })
            .await
            .map_err(|_| crate::Error::SendError)?;
        self.wait_recv_reply().await
    ...
    pub async fn channel_open_custom<K: Into<String>>(
        &self,
        kind: K,
...
impl Session {
    ...
    fn handle_msg(&mut self, msg: Msg) -> Result<(), crate::Error> {
        match msg {
            ...
            Msg::ChannelOpenCustom {
                kind,
                extra,
                channel_ref,
            } => {
                let id = self.channel_open_custom(&kind, &extra)?;
                self.channels.insert(id, channel_ref);
            }
            Msg::TcpIpForward {
                reply_channel,
            ...
            ) => self.request_custom(id, want_reply, &request, &payload)?,
            Msg::Channel(id, ChannelMsg::AgentForward { want_reply }) => {
                self.agent_forward(id, want_reply)?
            }
            Msg::Channel(id, ChannelMsg::Success) => self.channel_success(id)?,
            Msg::Channel(id, ChannelMsg::Failure) => self.channel_failure(id)?,
