                    ...
                    match buf.split_first() {
                        ...
                        Some((&msg::USERAUTH_INFO_REQUEST_OR_USERAUTH_PK_OK, mut r)) => {
                            ...
                            match self.common.auth_method.take() {
                                ...
                                | Some(auth_method @ auth::Method::OpenSshCertificateRaw { .. }) => {
                                    self.common.buffer.clear();
                                    enc.client_send_signature(
                                        &self.common.auth_user,
                                        &auth_method,
        ...
        match buf.split_first() {
            ...
            Some((&msg::CHANNEL_REQUEST, mut r)) => {
                ...
                match req.as_str() {
                    ...
                    _ => {
                        let wants_reply = map_err!(u8::decode(&mut r))?;
                        let payload = r.as_ref().to_vec();
                        if let Some(chan) = self.channels.get(&channel_num) {
                            let _ = chan
                                .send(ChannelMsg::CustomRequest {
                                    request: req.clone(),
                                    want_reply: wants_reply == 1,
                                    payload,
                                })
                                .await;
                        }
            ...
            Some((&msg::GLOBAL_REQUEST, mut r)) => {
                ...
                if let Some(ref mut enc) = self.common.encrypted {
                    ...
                    } else if req == "x-teleport-version" {
                        if wants_reply == 1 {
                            self.common.wants_reply = false;
                            push_packet!(enc.write, {
                                enc.write.push(msg::REQUEST_SUCCESS);
                                map_err!("16.4.12".encode(&mut enc.write))?;
                            });
                        }
                    } else {
...
impl Encrypted {
    fn write_auth_request(
        ...
        Ok(push_packet!(self.write, {
            ...
            match *auth_method {
                ...
                auth::Method::OpenSshCertificateRaw { ref algo, ref blob, .. } => {
                    user.as_bytes().encode(&mut self.write)?;
                    "ssh-connection".encode(&mut self.write)?;
                    "publickey".encode(&mut self.write)?;
                    self.write.push(0);
                    algo.encode(&mut self.write)?;
                    blob.as_slice().encode(&mut self.write)?;
                    true
                }
    ...
    fn client_make_to_sign(
        ...
        match key {
            ...
            PublicKeyOrCertificate::CertificateRaw { algo, blob } => {
                algo.encode(buffer)?;
                blob.as_slice().encode(buffer)?;
            }
    ...
    fn client_send_signature(
        ...
        match method {
            ...
            auth::Method::OpenSshCertificateRaw {
                ref key,
                ref algo,
                ref blob,
            } => {
                let i0 = self.client_make_to_sign(
                    user,
                    &PublicKeyOrCertificate::CertificateRaw {
                        algo: algo.clone(),
                        blob: blob.clone(),
                    },
                    buffer,
