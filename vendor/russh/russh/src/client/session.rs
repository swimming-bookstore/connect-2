...
impl Session {
        ...
        if let Some(ref mut enc) = self.common.encrypted {
            if let Some(channel) = enc.channels.get(&channel) {
                push_packet!(enc.write, {
                    msg::CHANNEL_REQUEST.encode(&mut enc.write)?;
                    channel.recipient_channel.encode(&mut enc.write)?;
                    request.encode(&mut enc.write)?;
                    (want_reply as u8).encode(&mut enc.write)?;
                    enc.write.extend(payload);
                });
            }
        }
        Ok(())
    ...
    pub fn channel_open_custom(
        &mut self,
        kind: &str,
        extra: &[u8],
    ) -> Result<ChannelId, crate::Error> {
        self.channel_open_generic(kind.as_bytes(), |write| {
            write.extend(extra);
            Ok(())
        })
    }
    ...
    pub fn channel_failure(&mut self, channel: ChannelId) -> Result<(), crate::Error> {
        if let Some(ref mut enc) = self.common.encrypted {
            if let Some(channel) = enc.channels.get(&channel) {
                push_packet!(enc.write, {
                    msg::CHANNEL_FAILURE.encode(&mut enc.write)?;
                    channel.recipient_channel.encode(&mut enc.write)?;
                });
            }
        }
