    ...
    pub fn step(
        ...
        match self.state {
            ...
            ClientKexState::WaitingForDhReply { mut names, mut kex } => {
                ...
                let server_host_key_blob = Bytes::decode(r)?; // server public key or OpenSSH cert.
                debug!(
                    "kex host key blob len={} prefix={:?}",
                    server_host_key_blob.len(),
                    &server_host_key_blob[..server_host_key_blob.len().min(48)]
                );
                let server_host_key = parse_public_key(&server_host_key_blob)?;
                debug!(
                ...
                // Hash the original host-key/cert blob (Teleport sends a host cert).
